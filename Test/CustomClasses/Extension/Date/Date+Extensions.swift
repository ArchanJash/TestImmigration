...
//  CQRest
//
//  Created by Thomas Richardson on 29/11/2017.
//  Copyright © 2017 CQRest. All rights reserved.
//
...
extension Date {
    ...
    func timeAgoSinceDate(numericDates:Bool) -> String? {
        ...
        let components:DateComponents = (calendar as NSCalendar).components(unitFlags, from: earliest, to: latest, options: NSCalendar.Options())
        
        if let componentYear = components.year, let componentMonth = components.month, let componentWeekYear = components.weekOfYear,let componentDay = components.day, let componentHour = components.hour,let componentMinute = components.minute, let componentSec = components.second {
            if componentYear >= 2 {
                return "\(componentYear) years ago"
            } else if  componentYear >= 1 {
                if numericDates {
                    return "1 year ago"
                } else {
                    return "Last year"
                }
            } else if componentMonth >= 2 {
                return "\(componentMonth) months ago"
            } else if componentMonth >= 1 {
                if numericDates {
                    return "1 month ago"
                } else {
                    return "Last month"
                }
            } else if componentWeekYear >= 2 {
                return "\(componentWeekYear) weeks ago"
            } else if componentWeekYear >= 1 {
                if numericDates {
                    return "1 week ago"
                } else {
                    return "Last week"
                }
            } else if componentDay >= 2 {
                return "\(componentDay) days ago"
            } else if componentDay >= 1 {
                if numericDates {
                    return "1 day ago"
                } else {
                    return "Yesterday"
                }
            } else if componentHour >= 2 {
                return "\(componentHour) hours ago"
            } else if componentHour >= 1 {
                if numericDates {
                    return "1 hour ago"
                } else {
                    return "An hour ago"
                }
            } else if componentMinute >= 2 {
                return "\(componentMinute) minutes ago"
            } else if componentMinute >= 1 {
                if numericDates {
                    return "1 minute ago"
                } else {
                    return "A minute ago"
                }
            } else if componentSec >= 3 {
                return "\(componentSec) seconds ago"
            } else {
                return "Just now"
            }
        }
